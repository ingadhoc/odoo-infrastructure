...
def send_mail(
        _from_, to_, subject, text, files=None,
...
class EmailParser(object):
    ...
    def __init__(
          self, uid, password, dbname, host, port,
          model=False, email_default=False):
    ...
    def parse(self, message, custom_values=None, save_original=None):
        # pass message as bytes because we don't know its encoding until we
        # fabtools and hence can't convert it to utf-8 for transport
